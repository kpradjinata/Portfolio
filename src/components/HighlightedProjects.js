      ...
      <div className="container mx-auto px-4">
        <div className="text-center md:text-left md:flex items-center justify-between mb-16">
            <div ref={headerRef} className="md:w-1/2 mb-8 md:mb-0 md:pr-8">
                <h2 
                  className={`text-4xl md:text-5xl font-bold text-blue-400 mb-6 uppercase tracking-wider transition-all duration-700 ease-out 
                              ${isHeaderInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}
                >
                  Spotlight
                </h2>
                <p 
                  className={`text-xl text-gray-300 leading-relaxed transition-all duration-700 ease-out delay-200 
                              ${isHeaderInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}
                >
        ...
        {Array.isArray(projects) && projects.length > highlighted.length && (
          <div ref={buttonRef} className="text-center">
            <Link 
              ...
              className={`inline-block bg-blue-500 text-white font-semibold px-10 py-4 rounded-lg hover:bg-blue-600 transition-all duration-500 ease-in-out text-lg shadow-md hover:shadow-lg transform 
