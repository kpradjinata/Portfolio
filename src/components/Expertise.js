...
const ExpertiseItem = ({ title, description }) => (
  <div className="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700 transform hover:scale-105 transition-transform duration-300 h-full flex flex-col">
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8"> {/* Adjusted grid for potentially more items */}
          {expertiseAreas.map(item => (
            <ExpertiseItem key={item.title} title={item.title} description={item.description} /* icon={item.icon} */ />
          ))}
        </div>
